# Perturbation Theory Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement deep zoom (10^-60+) via Rust/WASM arbitrary precision reference orbit + GPU float32 perturbation rendering with rebasing.

**Architecture:** Rust crate (`astro-float`) computes reference orbit at arbitrary precision, exports as `Float32Array`. GPU fragment shader iterates perturbation deltas from the reference orbit (stored as RGBA32F texture). Rebasing (Zhuoran 2021) handles glitches in-shader — no multi-reference needed. Automatic precision switching: float32 → DS → perturbation based on zoom depth.

**Tech Stack:** Rust + wasm-pack + wasm-bindgen, astro-float (arbitrary precision), WebGL 2 RGBA32F textures, Next.js WASM loading. WASM compiled with `+atomics,+bulk-memory` for SharedArrayBuffer-based cancel/progress.
...
### Modified files
- `src/domain/types.ts` — Add `DeepViewport`, `PrecisionMode`
...
- `src/application/useFractalState.ts` — Deep viewport string coordinates
- `src/application/useUrlState.ts` — Deep URL encoding
- `next.config.ts` — WASM webpack experiments
...
## Task 7: Shader Compiler Extension
...
Run:
```bash
npm test -- --run src/infrastructure/gpu/__tests__/shaderCompiler.test.ts
```
...
4. Update `assembleFragmentSource` signature and add perturbation path:
```typescript
...
5. Update `isGpuSupported` to accept precision:
```typescript
...
6. Update `getOrCompile` to pass precision through:
```typescript
export function getOrCompile(
  ...
  // ... rest of existing logic using updated assembleFragmentSource(fractal, coloring, maxIter, interiorColoring, precision)
}
```

- [ ] **Step 4: Fix any existing tests broken by signature change**

The existing tests call `assembleFragmentSource` with 4 args — the 5th arg defaults to `'doubleSingle'`, so they should still pass. Verify:
...
## Task 8: WebGL Renderer Extension
...
- [ ] **Step 1: Add orbit texture state and perturbation uniform binding**

In `webglRenderer.ts`, add to the `createWebGLRenderer` closure:
...
2. Add orbit state variables (inside `createWebGLRenderer`):
```typescript
let orbitTexture: WebGLTexture | null = null;
let orbitTexWidth = 0;
let orbitTexHeight = 0;
let currentOrbitLength = 0;
```

3. Extend `GPURenderOptions`:
```typescript
...
4. Add orbit uniform binding function:
```typescript
function setOrbitUniforms(
  locations: Map<string, WebGLUniformLocation>,
  orbitData: OrbitData
): void {
  const lengthLoc = locations.get('u_orbitLength');
  if (lengthLoc) gl.uniform1i(lengthLoc, orbitData.length);

  const texSizeLoc = locations.get('u_orbitTexSize');
  if (texSizeLoc) gl.uniform2f(texSizeLoc, orbitTexWidth, orbitTexHeight);

  const refPointLoc = locations.get('u_refPoint');
  if (refPointLoc) {
    const [reHi] = splitDouble(orbitData.refPointRe);
    const [imHi] = splitDouble(orbitData.refPointIm);
    gl.uniform2f(refPointLoc, reHi, imHi);
  }

  const refPointLoLoc = locations.get('u_refPointLo');
  if (refPointLoLoc) {
    const [, reLo] = splitDouble(orbitData.refPointRe);
    const [, imLo] = splitDouble(orbitData.refPointIm);
    gl.uniform2f(refPointLoLoc, reLo, imLo);
  }

  // Bind orbit texture to TEXTURE1 (palette is TEXTURE0)
  const orbitTexLoc = locations.get('u_orbitTexture');
  if (orbitTexLoc && orbitTexture) {
...
5. Add `uploadOrbit` method:
```typescript
function uploadOrbit(data: OrbitData): boolean {
  if (orbitTexture) {
    if (data.length <= orbitTexWidth * orbitTexHeight) {
      updateOrbitTexture(gl, orbitTexture, data.data, data.length, orbitTexWidth, orbitTexHeight);
      currentOrbitLength = data.length;
      return true;
    }
    destroyOrbitTexture(gl, orbitTexture);
  }
...
6. In the `render` method, pass `precision` to `getOrCompile` and call `setOrbitUniforms`:
```typescript
// In render():
const precision = options.precision ?? 'doubleSingle';
const compiled = getOrCompile(
  gl, options.fractalType, options.coloringMode,
  options.maxIterations, options.interiorColoring, precision
);
// ...
// After setCenterAndScale, before draw:
if (precision === 'perturbation' && options.orbitData) {
  if (!uploadOrbit(options.orbitData)) {
    // EXT_color_buffer_float unavailable (ISO 25010: compatibility fallback)
    // Fall through to DS/CPU path — caller should retry without perturbation
    return false;
  }
...
7. In `destroy()`, clean up orbit texture:
```typescript
...
npm test
```
Expected: All tests pass (existing functionality unchanged — new params are optional).
...
## Task 9: Pipeline Integration — Renderer Facade

**Goal:** Wire up the complete perturbation pipeline: zoom target → WASM orbit → GPU render.

**Files:**
- Modify: `src/infrastructure/renderer.ts`
- Modify: `src/infrastructure/useRenderer.ts`

- [ ] **Step 1: Extend renderer.ts with perturbation path**

In `src/infrastructure/renderer.ts`, add the perturbation pipeline:

```typescript
import { needsPerturbation, computeReferenceOrbit, loadWasm, isWasmReady } from './wasmBridge';
import type { OrbitData, PrecisionMode } from '../domain/types';

// In RenderOptions, add:
export interface RenderOptions {
  // ... existing fields ...
  /** Zoom target point for perturbation reference (mouse/touch position). */
  zoomTargetRe?: number;
  zoomTargetIm?: number;
}

// Helper to determine precision mode
function getPrecisionMode(viewport: Viewport, fractalType: FractalType): PrecisionMode {
  ...
  return 'doubleSingle'; // fallback for unsupported fractals
}
```

Then in `renderFractal`, before the GPU path:
```typescript
const precision = getPrecisionMode(options.viewport, options.fractalType);

if (precision === 'perturbation' && gpuRenderer) {
  // Start WASM load if not ready
  if (!isWasmReady()) {
    loadWasm().then(() => {
      // Re-trigger render after WASM loads
      renderFractal(canvas, pool, gpuRenderer, options);
    });
    // Fall through to DS/CPU for this frame
  } else {
    ...
    const refReStr = refRe.toString();
    const refImStr = refIm.toString();
    const scaleStr = options.viewport.scale.toString();

    computeReferenceOrbit(refReStr, refImStr, options.maxIterations, scaleStr)
      .then(({ data, length }) => {
...
- [ ] **Step 2: Pass zoom target from useCanvasEvents through to renderer**

In `src/infrastructure/useRenderer.ts`, pass the zoom focus point to `RenderOptions.zoomTargetRe/Im`. Read `useCanvasEvents.ts` to find where zoom actions provide the focus point, and thread it through the state.

This is a wiring step — the zoom focus coordinates already exist in `useFractalState` (from `ZOOM` action: `focusRe`, `focusIm`). Store the last zoom focus and pass it to the renderer.
...
- [ ] **Step 4: Manual test — first deep zoom**

Run:
```bash
npm run wasm:build && npm run dev
```

Open browser, zoom to ~10^-14 on Mandelbrot. Verify:
1. Console shows no errors
2. Perturbation path activates (check InfoPanel badge if wired)
3. Image renders without major artefacts at 10^-14 (overlap zone with DS)
...
## Task 10: UX — Progress, Cancel, Badges, Deep URLs
...
- Create or modify: UI component for InfoPanel (find via `grep -r InfoPanel src/`)

- [ ] **Step 1: Add orbit state to useFractalState.ts**

Add to the state interface and reducer in `src/application/useFractalState.ts`:

```typescript
import type { PrecisionMode } from '../domain/types';
import { getOrbitProgress, cancelOrbit } from '../infrastructure/wasmBridge';

// Add to FractalState interface:
...
In `src/application/useCanvasEvents.ts`, add to the keyboard handler:

```typescript
// Inside the keydown handler, add:
case 'Escape':
  cancelOrbit(); // from wasmBridge.ts
  break;
```

- [ ] **Step 3: Add orbit progress bar component (WCAG 2.1)**

Find the InfoPanel component (grep for it), add:
...
{state.orbitComputing && (
  ...
  >
    <div
      ...
      style={{ width: `${(orbitProgress / state.maxIterations) * 100}%` }}
    />
  </div>
)}
```

Where `orbitProgress` is polled from `getOrbitProgress()` via a `useEffect` + `requestAnimationFrame` loop while `orbitComputing` is true.

- [ ] **Step 4: Add precision mode badge (WCAG 1.4.1 — not color-only)**

```tsx
<span
  aria-live="polite"
  className="text-xs px-1.5 py-0.5 rounded border"
  title={state.precisionMode === 'perturbation'
    ? 'Deep zoom powered by perturbation theory — zoom deeper than 10⁻¹⁵'
    : undefined}
>
  {state.precisionMode === 'float32' && '32-bit'}
  {state.precisionMode === 'doubleSingle' && 'DS'}
  {state.precisionMode === 'perturbation' && 'Perturbation'}
</span>
```

Text label + border shape = distinguishable without color (WCAG 1.4.1).

- [ ] **Step 5: Add deep URL encoding to useUrlState.ts**

In `src/application/useUrlState.ts`, add deep viewport encoding:

```typescript
function encodeDeepViewport(viewport: Viewport): string {
  if (viewport.scale >= PERTURBATION_THRESHOLD) {
    // Existing float64 encoding
    return `re=${viewport.centerRe}&im=${viewport.centerIm}&s=${viewport.scale}`;
  }
  // Deep encoding: base64 decimal strings
  const dre = btoa(viewport.centerRe.toString());
  const dim = btoa(viewport.centerIm.toString());
  const ds = btoa(viewport.scale.toString());
  return `dre=${dre}&dim=${dim}&ds=${ds}`;
}
...
function decodeDeepViewport(params: URLSearchParams): Viewport | null {
  ...
  // Fall through to existing float64 decoding
  return null;
}
```
...
```bash
git add src/application/useFractalState.ts src/application/useCanvasEvents.ts \
  src/application/useUrlState.ts src/ui/
git commit -m "feat(ux): orbit progress bar, Escape cancel, precision badge, WCAG, deep URLs"
```

---

## Task 11: Playwright Benchmarks + Performance History
...
- [ ] **Step 1: Run Playwright benchmarks at various zoom depths**

Test coordinates:
- Overlap zone: `re=-0.75, im=0.1, scale=1e-13`
- Deep zoom: `re=-1.768778833, im=-0.001738996, scale=1e-20`
- Very deep: `re=-0.7436438885706, im=0.1318259043, scale=1e-40`

Measure: orbit computation time, GPU render time, total time.
...
|---|---|---|---|---|
| 10^-13 | Xms | Xms | Xms | Overlap zone |
| 10^-20 | Xms | Xms | Xms | |
| 10^-40 | Xms | Xms | Xms | |
| 10^-60 | Xms | Xms | Xms | |
```
...
## Future Tasks (documented, not yet implemented)

These are spec items 7-10, to be implemented after the core pipeline is validated:

### Series Approximation (SA) — Spec step 7
- Compute polynomial coefficients A_n, B_n in Rust
- Skip iterations on GPU using precomputed polynomials
- Expected: 5-10x speedup at deep zoom

### Progressive Orbit — Spec step 8
- Render partial orbit result while computation continues
- Re-render when full orbit available
- Target: <200ms to first visual at 10^-60

### Rescaling — Spec step 10
- Range extension when δ underflows in float32
- Substitute δ = S·w, iterate on w ≈ 1
- Only needed if artefacts appear at extreme relative zoom
...
- [ ] Manual test: zoom from overview to 10^-40 on Mandelbrot, no artefacts
- [ ] Manual test: zoom from overview to 10^-40 on Julia, no artefacts
- [ ] Manual test: BurningShip/Tricorn at 10^-14 show DS fallback gracefully
- [ ] Overlap zone (10^-13): perturbation matches DS output visually
- [ ] InfoPanel shows correct precision badge
- [ ] URL state preserved on page reload at deep zoom
- [ ] Escape key cancels orbit computation
- [ ] Progress bar has `role="progressbar"` + `aria-valuenow/min/max` (WCAG 2.1)
- [ ] Precision badge has `aria-live="polite"` (WCAG 2.1)
